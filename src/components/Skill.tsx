		...
		<comm.SectionWrap>
			...
			<SkillCardsWrap>
			{
				skillsData.map((skillItem, idx) => {
					return (
						<SkillCard key={idx} idx={idx} color={skillItem.color}>
							<SkillNameWrap idx={idx}>
...
export const SkillCard = styled.div<SkillCardProps>`
	...
	position: relative;
	${(props) => {
		return props.idx%2 === 0 ? 'padding: 30px 24px 30px 36px;' : 'padding: 30px 30px 30px 30px;'
	}};
	${(props) => {
    ...
    position: absolute;
		...
		${(props) => {
		return props.idx%2 === 0 ? 'border-top-left-radius: 30px; border-bottom-left-radius: 30px;' : 'border-top-right-radius: 30px; border-bottom-right-radius: 30px;'
		}};
		${(props) => {
			if( props.idx === 0){
				return 'border-top-right-radius: 30px;'
			}else if(props.idx === 5){
				return 'border-bottom-left-radius: 30px;'
			}
		}};
    top: 0px;
	}
...
`
export const SkillNameWrap = styled.div<SkillCardProps>`
	display: flex;
	column-gap: 15px;
	${(props) => {
		return props.idx%2 === 0 ? 'justify-content: left;' : 'justify-content: right;'
	}};
...
export const SkillDesc = styled.p<SkillCardProps>`
	width: fit-content;
	margin-top: 15px;
	margin-left: -6px;
	font-family: "Pretendard";
	...
	color: var(--gr-03);
	${(props) => {
		return props.idx%2 === 0 ? 'text-align: left;' : 'text-align: right;'
	}};
